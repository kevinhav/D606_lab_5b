...
**KH: In my sample, 63.3% believe climate change affects their local
community.**
...
**KH: No, I would expect another student's sample proportion to be
different as the sample they take would be different. They should be
somewhat similar.**
...
-   **Step 3.** Repeat steps (1) and (2) many times to create a
    bootstrap distribution - a distribution of bootstrap statistics.
-   **Step 4.** Calculate the bounds of the XX% confidence interval as
    the middle XX% j knof the bootstrap distribution.
...
**KH: 95% confidence means that we are 95% certain that the population
proportion lies within the given range. It does not say anything to
indicate any kind of probability.**
...
**KH: Yes, we've captured the true population proportion within our
bootstrap sample range (0.5 to 0.75). Another student is very likely to
also have captured this value, but given we are working with a 95%
confidence level, there is room for error to "miss" the population
proportion value.**

1.  Each student should have gotten a slightly different confidence
    interval. What proportion of those intervals would you expect to
    capture the true population mean? Why?

**KH: If we had 100 students all attempting to capture the true
population proportion, I would expect 95 of them to capture it in their
confidence interval, and 5 to miss it as we are using a 95% confidence
level.**
...
**47 out of 50 constructed confidence intervals capture the population
proportion. This is not exactly equal to the proportion we would expect
(94% compared to our 95% confidence level), but it is very close and we
can explain the 1% difference as a result of the "sample" of constructed
confidence levels having some randomness due to the underlying sampling
and bootstrapping process.**

```{r}
knitr::include_graphics("ci.png")
```


![confidence intervals](ci.png)
...
**A 99% confidence level would create a wider confidence interval, as we
need to be more certain that we've captured the population proportion -
we are casting a bigger "net".**

1.  Using code from the **infer** package and data from the one sample
...
**As expected, the 99% confidence level results in a wider range (0.433
to 0.75)**
...
samp %>%
  ...
  generate(reps = 1000, type = "bootstrap") %>%
  calculate(stat = "prop") %>%
  get_ci(level = 0.99) # changed to 99%
```
...
**We see all confidence level intervals capturing the population
proportion at 99% confidence level, which is greater than what we saw
for the 95% confidence level (as expected)**
...
**With a 75% confidence level, we missed the population proportion in 13
of our 50 confidence intervals. We can see the interval range is much
smaller, 0.517 to 0.65**
...
samp %>%
  ...
  generate(reps = 1000, type = "bootstrap") %>%
  calculate(stat = "prop") %>%
  get_ci(level = 0.75) # changed to 75%
```
...
**I actually broke the app trying to use a sample size less than 60!
Testing 100, 200, 300, 400, and 500, I noticed that the widths of
intervals decreased as sample size increased; an inverse relationship.**
...
1.  Finally, given a sample size (say, 60), how does the width of the
    ...
    **Hint:** Does changing the number of bootstrap samples affect the
    standard error?

**I did not see significant changes in the interval as bootstrapping
resamples increased. This is because bootstrapping "inherits" the
sampling error of the sample itself, which is derived from the
proportion and size of the sample only.**
